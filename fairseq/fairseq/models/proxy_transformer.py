...
class SimpleTransformerEncoder(TransformerEncoder):
    def __init__(self, args, dictionary, embed_tokens, EncoderModelLayer = None):
        encoder_layers = args.encoder_layers
        args.encoder_layers = 0
        super().__init__(args, dictionary, embed_tokens)
        self.layers = nn.ModuleList([])
        self.layers.extend([
            EncoderModelLayer(args)
            for _ in range(encoder_layers)
        ])
        args.encoder_layers = encoder_layers

class SimpleTransformerDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, DecoderModelLayer = None):
        decoder_layers = args.decoder_layers
        args.decoder_layers = 0
        super().__init__(args, dictionary, embed_tokens, no_encoder_attn)
        self.layers = nn.ModuleList([])
        self.layers.extend([
            DecoderModelLayer(args, no_encoder_attn)
            for _ in range(decoder_layers)
        ])
        args.decoder_layers = decoder_layers

class SimpleTransformerEncoderLayer(TransformerEncoderLayer):
    pass

class SimpleTransformerDecoderLayer(TransformerDecoderLayer):
    pass

class SimpleTransformerModel(TransformerModel):

    def __init__(self, encoder, decoder):
        super().__init__(encoder, decoder)

    @classmethod
    def build_model(cls, args, task, EncoderModel = SimpleTransformerEncoder, DecoderModel = SimpleTransformerDecoder, EncoderModelLayer = SimpleTransformerEncoderLayer, DecoderModelLayer = SimpleTransformerDecoderLayer):
        cls.EncoderModel = EncoderModel
        cls.DecoderModel = DecoderModel
        cls.EncoderModelLayer = EncoderModelLayer
        cls.DecoderModelLayer = DecoderModelLayer
        model = super().build_model(args, task)
        return model

    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens):
        return cls.EncoderModel(args, src_dict, embed_tokens, EncoderModelLayer = cls.EncoderModelLayer)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return cls.DecoderModel(args, tgt_dict, embed_tokens, DecoderModelLayer = cls.DecoderModelLayer)
#======================Proxy Transformer

MODEL_NAME = "proxy_transformer"

@register_model(MODEL_NAME)
class ProxyTransformerModel(BaseFairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        model = ProxyTransformerModel(tmodel)
        mode = {e.split('=')[0] : e.split('=')[1] if len(e.split('=')) > 1 else None  for e in args.user_mode.split(',')}
        model.user_mode = mode
        model.decoder = ProxyDecoder(tmodel, model.user_mode, args)
        model.encoder = ProxyEncoder(tmodel, model.user_mode, args)
        return model
    ...
    def evaluate_model(self, task, epoch_itr, trainer):
        ...
        sys.argv = args_bak[:1] + ("%s -s %s -t %s --path no_need --beam 5 --max-tokens %d --max-len-b 80 --results-path %s --remove-bpe --task %s"%(task.args.data, task.args.source_lang, task.args.target_lang, task.args.max_tokens // 8, log_file, task.args.task)).split()
...
class ProxyModule():# 不要继承nn.Module，不然model会有两份。
    def __init__(self, model, uer_mode, args, task = None):
    ...
    def get_src_words(self, src, replace_str = '@@ '):
        results = []
        for i in range(src.shape[0]):
            results.append(' '.join([self.model.encoder.dictionary.symbols[j] for j in src[i]]).replace(replace_str, ""))
        return results

    def get_tgt_words(self, tgt, replace_str = '@@ '):
        results = []
        for i in range(tgt.shape[0]):
            results.append(' '.join([self.model.decoder.dictionary.symbols[j] for j in tgt[i]]).replace(replace_str, ""))
        return results

    def get_words(self, tokens, replace_str = '@@ '):
        return self.get_tgt_words(tokens, replace_str)

    def _convert_padding(self, tokens, mode): 
        # "s2p", "p2s", "s2t", "p2t"
        # s: <pad>, ..., <pad>, <s1>, ..., <sn>, <eos>
        # p: <eos>, <s1>, ..., <sn>, <pad>, ..., <pad>
        # t: <s1>, ..., <sn>, <eos>, <pad>, ..., <pad>
        if mode == "s2p":
            tokens0, eos = tokens[:, :-1], tokens[:, -1:]
        elif mode == "p2s":
            eos, tokens0 = tokens[:, :1], tokens[:, 1:]
        elif mode == "s2t":
            tokens0 = tokens
        pad_id = self.pad
        mask = tokens0.ne(pad_id)
        tokens1 = tokens0.masked_select(mask)
        lengths = mask.sum(1)
        sents = tokens1.split(lengths.cpu().data.numpy().tolist())
        if mode == "s2p": # 
            tokens2 = pad_sequence([t for t in sents], padding_value = pad_id).transpose(1, 0)
            tokens3 = torch.cat([eos, tokens2], 1)
            lengths += 1
        elif mode == "s2t": #
            tokens3 = pad_sequence([t for t in sents], padding_value = pad_id).transpose(1, 0)
        elif mode == "p2s": #
            tokens2 = pad_sequence([t.flip(0) for t in sents], padding_value = pad_id).transpose(1, 0).flip(1)
            tokens3 = torch.cat([tokens2, eos], 1)
            lengths += 1

        return tokens3, lengths
